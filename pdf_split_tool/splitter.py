...
class LLMDocumentSplitter:
                    ...
                    try:
                        ...
                        if hasattr(paddle, 'is_compiled_with_cuda'):
                            ...
                            if paddle.fluid.is_compiled_with_cuda():
                                ...
                                self._gpu_available = True
                                return True
                        # GPU存在但PaddlePaddle不支持CUDA，仍然尝试使用GPU（PaddleOCR可能会处理）
                        logger.warning("检测到GPU但PaddlePaddle未编译CUDA支持，将尝试使用GPU（如果失败将自动降级到CPU）")
                        self._gpu_available = True
                        return True
                    except ImportError:
            ...
            if version.startswith('2.'):
                if use_gpu:
                    ...
                    self.ocr = PaddleOCR(
                        ...
                        enable_mkldnn=False,
                            cpu_threads=1,
                            ir_optim=True  # 启用推理优化，提升GPU性能
                        )
                        logger.info("✓ OCR引擎初始化成功（GPU模式，已启用推理优化）")
                    except Exception as gpu_error:
                        logger.warning(f"GPU模式初始化失败: {gpu_error}，降级到CPU模式")
                        self._gpu_available = False
                        # 降级到CPU配置
                        self.ocr = PaddleOCR(
                            use_angle_cls=True,
                        ...
                        logger.info("OCR引擎初始化成功（CPU模式，GPU降级）")
                else:
                    # CPU版本配置
                    self.ocr = PaddleOCR(
                        use_angle_cls=True,
                ...
                self.ocr = PaddleOCR(**ocr_kwargs)
                        logger.info(f"✓ OCR引擎初始化成功（GPU模式，3.x版本）")
                    except Exception as gpu_error:
                        logger.warning(f"GPU模式初始化失败: {gpu_error}，降级到CPU模式")
                        self._gpu_available = False
                        # 降级到CPU
                        ...
                        self.ocr = PaddleOCR(**ocr_kwargs)
                        logger.info("OCR引擎初始化成功（CPU模式，GPU降级，3.x版本）")
                else:
                    ocr_kwargs['use_gpu'] = False
                    self.ocr = PaddleOCR(**ocr_kwargs)
                    logger.info(f"OCR引擎初始化成功（CPU模式，3.x版本）")
        except Exception as e:
    ...
    def generate_ocr_json(self, output_json_path: Optional[str] = None) -> str:
        ...
        # 批量OCR处理
        ocr_results = {}
        # 根据GPU/CPU模式动态调整批处理大小
        if self._gpu_available:
            batch_size = 10  # GPU模式：使用更大的批次，充分利用GPU并行能力
        else:
            batch_size = 2  # CPU模式：使用较小的批次，避免内存问题
        
        logger.info(f"OCR批处理大小: {batch_size} (模式: {'GPU' if self._gpu_available else 'CPU'})")
    ...
    def _post_process_splits(self, splits: List[DocumentSplit], page_texts: List[Tuple[int, str, str, str]]) -> List[DocumentSplit]:
        ...
        if self.document_type in ("academic", "general"):
            ...
            for i, split in enumerate(corrected_splits):
                ...
                # 4.2 特殊处理：Appendix A, Appendix B 等格式
                if not should_merge:
                    import re
                ...
                # 4.3 智能合并：单页文档判断（新增）
                if not should_merge and pages_count == 1 and final_splits:
                    # 检查是否是正文片段（标题很短或像是正文内容）
                    title_len = len(split.title.strip())
                    is_fragment = False
                    
                    # 判断标准：
                    # 1. 标题很短（<20字符）且不包含明显的文档标识（如"通知书"、"申请书"、"判决书"等）
                    # 2. 标题像是正文片段（如"该案件符合立案的条件"、"综上所述"、"家里，在漏水"等）
                    # 3. 标题是时间戳、人名、简短描述等
                    fragment_keywords = ["综上所述", "该案件", "机关依据", "申请人", "家里", "漏水", 
                                        "掉落", "损坏", "拆改", "年月日", ":", "：", "的", "在"]
                    document_keywords = ["通知书", "申请书", "判决书", "决定书", "意见书", "证明书", 
                                       "委托书", "清单", "目录", "图纸", "条例", "证据"]
                    
                    has_document_keyword = any(kw in split.title for kw in document_keywords)
                    has_fragment_keyword = any(kw in split.title for kw in fragment_keywords)
                    
                    # 如果标题很短且不包含文档关键词，或者包含片段关键词，可能是正文片段
                    if (title_len < 20 and not has_document_keyword) or (has_fragment_keyword and not has_document_keyword):
                        is_fragment = True
                    
                    # 4.4 检查前面是否有连续的单页文档（更保守的策略）
                    if is_fragment:
                        # 检查前面连续的单页文档数量
                        consecutive_single_pages = 0
                        for j in range(len(final_splits) - 1, -1, -1):
                            prev_pages = final_splits[j].end_page - final_splits[j].start_page + 1
                            if prev_pages == 1:
                                consecutive_single_pages += 1
                            else:
                                break
                        
                        # 更保守的合并策略：
                        # 1. 只有当前面有3个以上连续单页片段，且当前也是片段时，才合并
                        # 2. 如果前面文档包含文档关键词（如"申请书"、"通知书"等），不合并（可能是独立文档）
                        if consecutive_single_pages >= 3 and is_fragment:
                            # 检查前面的文档是否包含文档关键词
                            prev_has_doc_keyword = False
                            if final_splits:
                                prev_title = final_splits[-1].title
                                prev_has_doc_keyword = any(kw in prev_title for kw in document_keywords)
                            
                            # 如果前面的文档包含文档关键词，说明可能是独立文档，不合并
                            if not prev_has_doc_keyword:
                                should_merge = True
                                logger.info(f"检测到连续单页片段：{split.title}（第{split.start_page}页），合并到前面的文档（前面已有{consecutive_single_pages}个单页）")
                        # 不再合并到多页文档（避免错误合并）
                
                # 执行合并
                if should_merge and final_splits:
                    prev_split = final_splits[-1]
                    logger.info(f"合并文档：{split.title}（{pages_count}页）→ {prev_split.title}")
...
#### 2.2 可以合并的情况（非常谨慎使用，优先拆分）
- **几乎不合并**：除非非常确定是同一文档的不同部分，否则应该拆分
- 只有明确的补充材料（如"Appendix A"、"参考文献"等）才考虑合并
- **多页文档特别注意**：如果某个文档超过3页，很可能包含了多个应该分开的文档，需要仔细检查是否有多个标题或文档边界

**重要原则：优先拆分，谨慎合并！**
- **优先拆分**：每个有明确标题的文档都应该独立拆分，即使页数很少（1-2页）
- **不要过度合并**：只有在非常确定的情况下才合并，避免将独立文档错误合并
- **单页文档处理**：单页文档通常应该保持独立，除非：
  - 标题非常短（<10字符）且明显是正文片段（如"综上所述"、"该案件"等）
  - 连续出现3个以上单页片段，且都不包含文档关键词（如"通知书"、"申请书"等）
- **包含文档关键词的单页文档**：如果单页文档包含"通知书"、"申请书"、"判决书"、"决定书"、"意见书"、"证明书"、"委托书"、"清单"等关键词，必须保持独立，不能合并
- **补充材料合并**：只有明确的补充材料（如"Appendix A"、"参考文献"等）才合并到前面的文档
